...
export default class GUI {
    constructor(websocketsConnection) {
        this.websocketsConnection = websocketsConnection;

        this.registerSocketEvents(websocketsConnection);
    ...
    registerSocketEvents(websocketsConnection) {
        websocketsConnection.websocket.onmessage = this.onMessage.bind(this);
    }

    sendMessage(type, currentWord) {
       this.websocketsConnection.sendWord(currentWord);
    }
    ...
    showWord(mr) {
        let endOfGame = () => {
            ...
            if (this.wordle.isOver) {
                ...
                window.onkeyup = undefined;
                message.textContent = `Você perdeu! ${this.wordle.game.secret.toUpperCase()}`;
